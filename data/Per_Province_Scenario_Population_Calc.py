...
# TIF_PATH = "Population Projection Gridded 2/SSP2RCP4_5/grid_pop_count2020_SSP2_RCP4_5.tif"           
# OUT_PATH = "Emission Files/pop_grid_wrf_2020.npy"                       
# HALF_WRF_DEG = 0.18
...
for file in nc_files:
    ...
    percentage = round((pop_china_2020[mask].sum() / pop_china_2020[in_china].sum()) * 100, 2)
    ...
    dictionary[scenario].append(percentage)
    
    if (len(dictionary[scenario]) == 8):
        print(f"Scenario: {scenario}, Percentage: {np.mean(dictionary[scenario])}")

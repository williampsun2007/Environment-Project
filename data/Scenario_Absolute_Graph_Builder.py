'''
Builds an 8x5 grid of stacked bar charts (pollutant x scenario)
showing sector emission breakdown by year, and saves the combined
figure as one PNG.
'''
...
for i, pollutant in enumerate(["SO2", "NOx", "VOC", "NH3", "PM25", "PM10", "BC", "OC"]):
    for j, scenario in enumerate(["Baseline", "CleanAir", "OTPCA", "OTPNZCA", "EPNZCA"]):
        ...
        for department in pivot.columns:
            ax.bar(x, pivot[department].values, bottom = bottom, color = department_colors[department])
            bottom += pivot[department].values

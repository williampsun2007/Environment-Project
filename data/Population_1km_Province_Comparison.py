...
# Bonus: national total row from the table (not a province, so kept separate)
...
for idx, province in enumerate(province_list):
    ...
    fig.add_trace(go.Scatter(x = list(range(2015, 2025)), y = china_population_dataset[province], mode = "markers", 
        name = "Actual", marker = dict(size = 6, color = 'black'), visible = visible))
